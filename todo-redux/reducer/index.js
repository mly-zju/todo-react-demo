...
function reducer_item(state = [], action) {
  switch (action.type) {
    case 'ADD_ITEM':
      var tmp = [];
      tmp = state.slice(0);
      tmp.push({text: action.text, done: false, id: action.id});
      return tmp;
    case 'TOGGLE_ITEM':
...
const combine_reducer = combineReducers({items: reducer_item, text: reducer_inputText, filter: reducer_filter});
